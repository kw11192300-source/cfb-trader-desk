...
-- In-season "wait for confirmation" watchlist (python/modeling/watchlist.py).
-- EXPLORATORY / UNVALIDATED, separate strategy from predictions'
-- week1_edge_v1 - see watchlist.py's module docstring. Backtest finding
-- this is based on (2016-2025, post-week-1 FBS-vs-FBS): the model's raw
-- top-15-by-edge picks are ~break-even (53-56% ATS) once a team has
-- played a game, BUT the subset where the closing line later moves
-- toward the model's side went 64% ATS vs. 41% when it didn't - CLV
-- direction is a real, correlated-with-winning signal post-week-1 (unlike
-- week 1 itself, where it isn't). A row is created once a candidate game
-- first clears CANDIDATE_EDGE_THRESHOLD (reference_spread = the market
-- line at that moment, fixed from then on) and alerted once the CURRENT
-- line has moved >= CONFIRM_MOVE_THRESHOLD toward the pick.
create table if not exists watchlist_picks (
  ...
  pick_home boolean not null,
  predicted_margin numeric not null,
  edge numeric not null,                    -- edge vs. the CURRENT line as of the last scan (updates each run)
  rationale text,

  reference_spread numeric not null,        -- fixed at discovery - never overwritten
  reference_captured_at timestamptz not null default now(),
  current_spread numeric,                   -- updated every scan
  current_edge numeric,
  move_toward_pick numeric,                 -- current_spread vs. reference_spread, in the pick's favor (points)
